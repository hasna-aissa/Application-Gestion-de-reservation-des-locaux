...
<div id="table-scroll" class="table-scroll">
  <div class="table-wrap ex1">
    <table class="main-table table-bordered table table-striped">
      <thead>
        <tr>
          <th style='background:#A2C6F1;' >Salles/Jours</th>
                  <?php 
                 ...
                 $dateauj=date('l');
                  $queryDate=$conn->query("SELECT Date FROM creneu GROUP BY Date");
                   ...
                   while ($line9=$queryDate->fetch()) {
                    ...
                    if ($day!='Sunday') {
                      echo"<th style='background:#A2C6F1;'>".$day." le ".$line9['Date']."</th>";
                    }
                    ...
                    $queryLocal=$conn->query("SELECT * FROM local where NomLocal LIKE '%".$str."%' ");
      ...
      <tbody>
         <?php 
                ...
                foreach ($lineLocal as $row) { 
                  $l= $row['IdLocal'];
                  echo'</tr>';
                echo"<tr>";echo"<td style='background:#A2C6F1;font-size:30px'>".$row['NomLocal']."</td>";
                for ($k=1; $k <=$nombre ; $k++) { 
