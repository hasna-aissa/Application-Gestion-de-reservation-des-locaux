...
if ($day=='Sunday' || $day1=='Sunday') {
	header('Location:ReserverSalle.php?erreur=65');
}
else
{
	if ($dateauj>$datedebut || $dateauj>$datefin  ) {
	header('Location:ReserverSalle.php?erreur=60');
	
}
else
{
	if (($timeauj>$timedebut || $timeauj>$timefin) && ($dateauj>$datedebut || $dateauj>$datefin  ))  {
		header('Location:ReserverSalle.php?erreur=61');
	}
	else
	{
			if ($table[0]<=$table2[0]) {
...
if ($rep['count(*)']!=0) {
	header('Location:ReserverSalle.php?erreur=100');
}
else{
...
$stmt->execute([$professeur, $sujet, $filiere, $grp_sec,$niveau,$timedebut,$timefin,$datedebut,$datefin,$idLocal]);
  ...
  header('Location:ReserverSalle.php');
	...
	header('Location:ReserverSalle.php?erreur=5');
}
}
		 ...
		 header('Location:ReserverSalle.php?erreur=30');
	}
	}
}
